...
let animate=document.querySelector("#display-wether");
let animation=window.getComputedStyle(animate, ':after');
...
if (select.value == "Pune") {
 ...
 let info_changed = temp.innerText = url_temp[time_temp] + 15
...
if (info_changed >= 20 && info_changed <= 30) {
 if (myWidth <= 420) {
   ...
   info_changed=info_changed.toFixed(4);
...
let wind_box=document.getElementById("wind_box")
let humidity_box=document.getElementById("humidity_box")
...
color_mode.addEventListener("click", ()=>{
  body.style.backgroundImage="url('images/dark_mode.png')"
   ...
   wind_box.style.boxShadow="none"
   humidity_box.style.boxShadow="none"

      let dw = document.getElementById("display-wether")
     let css=window.getComputedStyle(dw,'::after')
    console.log(css)
    console.log(css.backgroundColor);
    let dw2=dw.style.setProperty('--after_color', 'white');
    console.log(dw2)
